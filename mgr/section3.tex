...
Wprawdzie konstrukcji funkcji haszujących jest wiele, \renmich{jednak} większość z~nich wygląda
...
kolizji):\renmich{To jest nie po polsku; może ,,próbuje dobrać kolejne losowe elementy póki nie trafi na jeszcze niewykorzystany'', czy coś takiego?}
...
\begin{myenumerate}
    ...
    \begin{myenumerate}
        ...
        h$\renmich{Sugeruję dodanie $\backslash$! między $\backslash$not a $\backslash$exists}
...
\begin{itemize}
    ...
    używają klucza).\renmich{Ale jeśli ukryjemy tablicę i potraktujemy algorytm jako black box, to jest to dokładnie random oracle, prawda? Nie wiem, czy to należy za wielką wadę uznać; można o tej tablicy wręcz myśleć jak o kluczu\ldots}
    ...
    wewnątrz $m \in M$, co może doprowadzić do oryginalnych $m$.\renmich{Chyba głupi jestem, bo zupełnie nie rozumiem jak to może zachodzić. Mnie się ten algorytm wydaje całkowicie odporny przed takim atakiem.}
...
\begin{itemize}
    ...
    prawdopodobieństwem~\cite{one_way_functions}.\renmich{Z zaniedbywalnie małym prawdopodobieństwem chyba właśnie potrafi.}
...
\begin{itemize}
    ...
    z~niezaniedbywalnie dużym prawdopodobieństwem~\cite{one_way_functions}.\renmich{Wydaje mi się to nieprawdziwe, a w zasadzie niewystarczające. Zamiast czytać \cite{one_way_functions}, pozwolę Panu mnie przekonać, że jest inaczej. :)}
...
szybkiego rozwiązywania problemu, na którym ta funkcja się opiera~-- a~na\renmich{w chwili obecnej}
...
dowolnego\renmich{to by sugerowało, że chodzi o NP-zupełne; wyrzuciłbym ,,dowolnego''} problemu, o~którym podejrzewamy, że jest trudny do rozwiązania.
...
przychodzi to bez kosztów\renmich{to nie jest po polsku}. Praktyka pokazuje, że funkcje te prawie w~ogóle się
...
skrótu. Działanie funkcji z~tej rodziny polega\renmich{przecinek} ogólnie rzecz biorąc\renmich{przecinek} na
dokonywaniu ogromnych\renmich{ogromnej} liczby różnorodnych operacji na konkretnych bitach
...
nieznalezienie igły na daną chwilę\renmich{w danej chwili} nie oznacza jeszcze nieistnienia igły. Daje
to pole do popisu kryptologom, którzy znajdują luki na\renmich{usunąć ,,na''} długo po pierwszym
opisaniu.
...
raz pierwszy przez Merkle\renmich{Merkle'go?} w~roku 1979 w~jego pracy
...
\begin{myenumerate}
    ...
    \begin{myenumerate}
        ...
        funkcją kompresji, $h$ aktualnie obliczoną wartość\renmich{wartością} skrótu a~$M_i$
...
że już w~latach 90-tych wyraźnie odradzano jej wykorzystania\renmich{wykorzystanie} z~powodu
