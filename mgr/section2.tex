...
\begin{itemize}
    ...
    \item \textbf{wiadomość}~-- wartość wejściowa dla\renmich{czy to ,,dla'' jest gramatycznie?} \emph{funkcji skrótu},

    \item \textbf{skrót}~-- wartość wyjściowa z\renmich{Czy to ,,z'' jest gramatycznie?}~\emph{funkcji skrótu},

    \item \textbf{hasz}~-- alternatywne określenie na\renmich{niegramatycznie} wartość wyjściową
    \emph{funkcji skrótu},
    ...
    realizuje dane zadanie,\renmich{usunąć przecinek} nie istnieje, albo jego koszt obliczeniowy lub
...
skupiać.\renmich{Szczerze powiedziawszy, to dobrze by było konsekwentnie dalej wszystkie nazywać, nie zmuszając czytelnika, żeby to pamiętał\ldots}
\pagebreak
...
skrótu przyporządkowuje dokładnie jedno wyjście; \renmich{Naprawdę bycie suriekcją oznacza, że przyporządkowanie jest jednoznaczne?} formalnie zatem jest to
surjekcja:
...
też \emph{bezpiecznych}\renmich{Przecinki mi się nie podobają} funkcji skrótu, należy uświadomić sobie ich naturę:
...
Jednak \renmich{Przecinek}z~racji mniejszego rozmiaru przeciwdziedziny w~stosunku do dziedziny
...
\label{sec:preimage_resistance}\renmich{Dokładnie w tym miejscu należy wpisać \%, bo inaczej będzie spacja na początku linii}
\renmich{Może lepiej}Mając dany skrót $h$, znalezienie wiadomości $m$ takiej, że $H(m) = h$ powinno
...
resistance})}
\label{sec:second_preimage_resistance}\renmich{Dokładnie w tym miejscu należy wpisać \%, bo inaczej będzie spacja na początku linii}
...
\label{sec:collision_resistance}\renmich{Dokładnie w tym miejscu należy wpisać \%, bo inaczej będzie spacja na początku linii}
...
odporność na kolizje drugiego rzędu, co\renmich{usunąć ,,co'', albo napisać inaczej, bo teraz jest że implikacja nie implikuje, a nie spełnienie własności} jednak nie implikuje odporności na
...
choć jest to zadanie znacznie trudniejsze od spełnienia\renmich{zapewnienia?} poprzednich własności.

\myparagraph{Własność efektu lawinowego}
\label{sec:avalance_effect}\renmich{Dokładnie w tym miejscu należy wpisać \%, bo inaczej będzie spacja na początku linii}
...
\begin{myenumerate}
    ...
    kryterium lawinowe, gdy:\renmich{,,j'' tu nie występuje}
    ...
    niezależności bitowej. Formalnie:\renmich{Trzeba zdefiniować corr. Poza tym trzeba się zastanowić czy tu i dalej corr i BIC mają być kursywą -- można by zrobić $\backslash$textrm\{BIC\}.}
...
\begin{figure}[hbt!]
    ...
    \end{figure}\renmich{Zahaszowania?}\renmich{Nie rozumiem indeksu dolnego w opisie osi y}
\pagebreak
...
W~każdym przypadku opisane zastosowanie utylizuje\renmich{Utylizować to odpady można.} podstawową cechę
...
\end{figure}\renmich{identycznej do, nie jak}
...
Wprowadzenie funkcji haszujących właśnie\renmich{,,właśnie'' na początek?} pozwoliło sprowadzić problem
...
Wyobraźmy sobie przeciętny\renmich{typowy} mechanizm autentykacji.
...
inaczej zabezpieczać dane w~niej przechowywane,\renmich{kropka} problem z\renmich{,,problem z czymś'' to kolokwializm}~takim podejściem jest
jednak taki, że na pewnym poziomie tak czy inaczej jakaś osoba (administrator)
zawsze będzie miał\renmich{miała} dostęp do fizycznych danych. Możemy także zahaszować\renmich{zahaszować?} dane
...
powinno być można\renmich{dać się?} odtworzyć oryginalnych danych (patrz
...
poprawek. Przyczyny\renmich{tego?} i~natura poprawek omówione są
...
\begin{itemize}
    ...
    \item Linki\renmich{odnośniki, albo może adresy} ed2k oraz magnet, używane w~sieciach peer-to-peer, wykorzystują
    funkcje skrótu w~celu lokalizacji źródeł,\renmich{bez przecinka} skąd dany plik może być pobrany,
    ...
    dowolnych obiektów na taką postać\renmich{do takiej postaci}, jaką ich algorytmy indeksowania są
    ...
    możliwe) konkretne instancje obiektu w~stosunku do\renmich{w stosunku do $\rightarrow$ od?} pozostałych.
...
Już od samego początku, czyli późnych lat 70--tych\renmich{Ktoś lepszy ode mnie w typografii powinien się wypowiedzieć czy nie powinno być 70-tych. Zapytam dr Borkowskiego.}, funkcje haszujące
