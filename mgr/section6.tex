...
metodą obrony i\renmich{myślnik?}~jest nią identyfikacja atakującego i~następujące po niej
...
ograniczenie liczby możliwości wypróbowania kolejnych kombinacji w~\renmich{ustalonym odcinku? (bez stosunku)}stosunku do
...
jest wolny\renmich{powolny? (mniejsza niejednoznaczność)} z~powodu ograniczeń przepustowości sieci, a~\renmich{zidentyfikowanie atakującego\ldots jest trywialne}atakujący
...
a~więc nie\renmich{?} coraz trudniejsze staje się efektywne odebranie dostępu atakującemu
do danej usługi. Można bezpiecznie założyć, że poważny atakujący będzie
dysponował wszystkimi tymi zasobami.

Tak naprawdę problem zapobiegania atakom o~poważniejszej\renmich{powtórzenie} skali sprowadza się do
...
W~kontekście kryptograficznych funkcji skrótu istnieje kilka kontekstów\renmich{powtórzenie},
w~których można mówić o~ataku DDoS.

\begin{itemize}

\item Funkcja haszująca jest wystarczająco wolno działająca\renmich{działa wystarczająco powoli?}, że nawet niewielka
ilość\renmich{liczba!} zapytań potrafi wyczerpać zasoby serwera.

\item ``Klasyczny'' atak DDoS~-- zasoby serwera są wyczerpane przez dużą ilość\renmich{liczbę!}
...
i~przygotowywać specjalne zapytania, które będą powodować wolne\renmich{powolne? i dalej może też\ldots} działanie
...
konkretnych rewizji\renmich{rewizja to po polsku coś innego :)} kodu).
...
działanie algorytmów i~przekształcać je na bezpieczne odpowiedniki. \renmich{Jednak także?} Także taka
...
można\renmich{nic na to poradzić} z~tym nic zrobić, sytuacja sprowadza się w~zasadzie do zabezpieczenia
...
hasło powinien być względnie bezpieczny w~sytuacji, kiedy baza\renmich{danych z hasłami} zostanie
wykradziona.

\subsubsection{Konkatenacja haszy}

Jednym z~pomysłów jest próba zwiększenia siły funkcji skrótu bez jej
faktycznego zmieniania stosując\renmich{przez zastosowanie \ldots} konkatenację. Zamiast przechowywać w~bazie
hasz postaci:
    $$H(m)$$
przechowywane są hasze postaci:
    $$H_1(m)||H_2(m)||\ldots||H_n(m)$$
gdzie $H_i$ to kolejne\renmich{przecinek} różne od siebie\renmich{przecinek} funkcje haszujące. Dawne protokoły
...
zagwarantować\renmich{zagwarantowanie} nieskuteczność ataków przy pomocy tęczowych tablic oraz ataków na
...
jednak zwrócić uwagę że przyrost ten nie polepsza \renmich{znacząco} bezpieczeństwa;
...
soli. Zabezpieczenie to\renmich{przecinek} w~przeciwieństwie do poprzedniej metody\renmich{przecinek} zakłada, że
...
$\texttt{MD5}$, nie daje mu to\renmich{posiada} znaczącej przewagi, gdyż do przeprowadzenia
...
to niestety z~paroma\renmich{kilkoma? paroma to chyba zbyt kolokwialne\ldots} wadami.
\begin{enumerate}
...
attack}~(\ref{length_extension_attack}).
...
\end{enumerate}
Jeżeli jednak jesteśmy\renmich{czegoś brakuje} zagwarantować bezpieczeństwo $s$ (np. poprzez
zastosowanie w~tym celu osobnego systemu kryptograficznego), podejście to
w~połączeniu z~zastosowaniem~\ref{salt_1} stanowi najlepsze z~przedstawionych. \renmich{\ldots{}ale można by chyba wspomnieć choć słówkiem o tym, co naprawdę powinno się używać, czyli key derivation functions jak np. scrypt; wręcz można by o tym napisać nawet więcej niż słówko, jeśli Pan chce}
