...
mierzą w~słabości\renmich{wykorzystują słabości?}\renmich{biorą na cel słabości} konkretnych implementacji funkcji haszujących.
...
ilość\renmich{liczbę!} skrótów daną wzorem:
...
przedstawia ilości\renmich{liczby!} haszy, które należy wygenerować, żeby otrzymać kolizje
...
ataku, przedstawiona poniżej, stosowana jest m.in. na\renmich{Nie wiem czy ,,na'' jest tu dobrym przyimkiem.} \texttt{MD4},
...
wystarczających, aby $M$ i~$M'$ dawały kolizję. Warunki te formułują\renmich{ustalają?}, jakie
...
attack}, nie gwarantuje\renmich{powoduje to koniecznie? pociąga?} to jej kryptograficznej bezużyteczności. Gdy funkcja
