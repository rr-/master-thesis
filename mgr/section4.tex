...
Przykłady będą odnosiły się do scenariuszu\renmich{scenariusza} opisanego
...
atakujący ma częściowy lub kompletny dostęp\renmich{do} bazy danych zawierającej dane takie
...
wypróbowana\renmich{niegramatycznie} wraz z~upływem czasu, bez znacznego faworyzowania jakichkolwiek
...
\begin{myenumerate}
    ...
    przypominającej losową, jest trudna w~implementacji\renmich{implementacja trudna w implementacji} oraz kosztowna
    ...
    wyciągnięcia\renmich{wyciągnięcia?} odpowiednich liter od prawdopodobieństwa ich wystąpienia
...
Dodatkowo\renmich{Powtórzenie. ,,Ponadto''?} w~przypadku gdy atakujący obiera na cel konkretny system, może on
...
\begin{itemize}
    ...
    \item
        ...
        Liczba generowanych haseł: $n!$.\renmich{Gdzie n to co?}
        %todo: kod?

    \item
        Zapisanie hasła podwójnie \\
        Liczba generowanych haseł: 2. \renmich{Czyli próbujemy zhaszować dwa razy, na wszelki wypadek?}
        %todo: kod?
    ...
    \item
        ...
        %todo: kod?
\renmich{Zdecydowanie bym dopisał kod, nawet niby trywialnych przypadków. Opis nie zawsze jest jasny.}
...
hasła z~dopisanym na końcu znakiem ``1''\renmich{nipolski cudzysłów przy ,,1''} wydłuża rozmiar słownika dwukrotnie.
...
\begin{myenumerate}
    ...
    szybkie bez względu na $|M|$ i~ilość\renmich{liczbę} niezależnych ataków. Jednak aby
...
Budując tablicę\renmich{Przy budowie tablicy\ldots -- inaczej jest ,,idąc do domu, spadł deszcz''.} mającą na celu przechować zbiór kandydatów $M$, wybierany jest
...
$\mathtt{aaaaa}$ oraz $\mathtt{zqjwc}$); nie zapisywane są żadne \renmich{żadne pośrednie}z~pośrednich
wartości.
...
\in C_E$, zatem należy przyjrzeć się dokładniej łańcuchowi, \renmich{na}którego końcówkę
...
\columnbreak
    ...
    \]
\end{multicols}
\renmich{Dlaczego prawe hasze są niżej?}
...
długie (i~monotonne) z~uwagi na konieczność sprawdzania dużych liczb\renmich{,,wielu''?}
...
tablic tęczowych jest dużo bardziej opłacalne z~racji na zminimalizowane\renmich{,,zmniejszenie liczby''? trochę nie po polsku}
kolizje.
...
technik racjonalizujących\renmich{Czyli jak tłumaczyć szefowi, że mimo wszystko trzeba to zrobić? Obawiam się, że może być dwuznaczne\ldots}\renmich{Chodziło mi tu o to, że ,,techniki racjonalizujące'' to (również) techniki uzasadniania i w języku polskim wydaje mi się, że to znaczenie jako pierwsze się narzuca.} czas potrzebny na przeprowadzenie kosztownych
obliczeń.
...
\begin{itemize}
    ...
    kształtującą się kwestią;\renmich{przecinek?} dlatego też usługodawcom tego typu trudno
...
Podejście to jest o~tyle lepsze od korzystania z~dedykowanego \enn{hardware}\renmich{może ,,sprzętu''?},
...
of Service}. W~przeciwieństwie do poprzednich ataków, atak ten nie ma\renmich{nie ma on} na celu
wykradzenia oryginalnych wejść\renmich{trochę nie po polsku brzmi} do funkcji haszujących; jego celem jest
...
\begin{itemize}
    ...
    Wiele struktur danych\renmich{przecinek?} takich jak tablice mieszające\renmich{przecinek?} korzysta z~funkcji
    ...
    ilości\renmich{liczby!} zapytań odwołujących się do $m$~i~$m'$. Ponieważ struktury danych
