...
\begin{itemize}
    ...
    \item
        ...
        \lstinputlisting[language=python,caption=Przykładowy kod
        w~Pythonie]{code/mutate_suffix.py}
    ...
    \item
        ...
        \lstinputlisting[language=python,caption=Przykładowy kod
        w~Pythonie]{code/mutate_infix.py}
...
potrafią w~dzisiejszych czasach przetworzyć nawet do kilku milionów haszy na
sekundę~\cite{oclhashcat,john_the_ripper_benchmarks}. Gdy atakujący zaprzęgnie
do przeszukiwania przestrzeni haszy jeszcze $n$ dodatkowych komputerów, czas
przeszukiwania skróci się $n$-krotnie. Z~pomocą w~praktycznej implementacji
takiego ataku przychodzi parę rozwiązań.
...
\begin{itemize}
    ...
    \item Legalne platformy chmurowe takie jak \enn{Windows Azure}, \enn{Amazon
    Elastic Compute Cloud (EC2)} czy też \enn{Rackspace}. Stopień prywatności
...
szybciej.  Rozwiązanie to jest także dużo tańsze. Nie jest ono jednak
pozbawione wad~-- gdy atakujący korzysta z~maszyn, nad którymi ktoś inny
sprawuje faktyczną kontrolę, zdemaskowanie staje się znacznie bardziej
prawdopodobne.
