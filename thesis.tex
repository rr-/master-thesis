...
\begin{figure}[hbt!]
    ...
    kolejnych bitach skrótów \texttt{MD5}, otrzymanych z~zahaszowania 390532
    wyrazów z~załącznika~\ref{att:english_wordlist}.}
\end{figure}
\pagebreak
...
\begin{myenumerate}
    ...
    \begin{myenumerate}
        ...
        \footnote{Wprawdzie wydawałoby się, że we wcześniejszym kroku mogliśmy
        zawinąć wiadomość do długości po prostu podzielnej przez 512~bitów,
        a~w~aktualnym kroku nadpisać dodane zera, jednak takie podejście
        spowodowałoby, że moglibyśmy nadpisać bajty oryginalnej wiadomości $m$.
        Dlatego też w~powyższych dwóch krokach zadbaliśmy nie tylko, aby
        długość wiadomości po dopisaniu jej długości była podzielna przez 512,
        ale też aby nie nadpisano żadnych ważnych bitów.}
    ...
    \begin{myenumerate}
        ...
        jest inicjowany stałym wektorem $\mathtt{IV}$, co przekłada się na
        przydzielenie $A$, $B$, $C$ i~$D$ następujących wartości:
        ...
        \item Następnie dla każdego kolejnego bloku wiadomości $M_0, M_1, M_2,
        \ldots, M_n$ będziemy aktualizować wewnętrzny stan $(A,B,C,D)$
        przypisując mu w~każdym kroku wartość funkcji kompresji obliczoną na
        podstawie danego bloku i~poprzedniego stanu:
        $(A,B,C,D) := \mathtt{Comp}((A,B,C,D),M_i)$.

        \pagebreak
        Funkcja kompresji składa się z~4~rund. W~każdej rundzie wykonywane jest
        16~operacji przekształcających stan $(A,B,C,D)$, przedstawionych na
        rysunku~\ref{fig:md5_operation}. Każda operacja wykorzystuje funkcję
        $F$, która jest zależna od aktualnej rundy w~następujący sposób:

        \begin{itemize}

            \item runda 1 -- $F(B,C,D) = (B \land C) \lor (\lnot B \land D)$,
            \item runda 2 -- $F(B,C,D) = (B \land D) \lor (C \land \lnot D)$,
            \item runda 3 -- $F(B,C,D) = B \oplus C \oplus D$,
            \item runda 4 -- $F(B,C,D) = C \oplus (B \lor \lnot D)$.
        ...
        \begin{itemize}
            ...
            \item $\land$ -- operator koniunkcji,
            \item $\lor$ -- operator alternatywy,
            \item $\oplus$ -- operator alternatywy wykluczającej.

        \end{itemize}

        Powyższe operatory przypisują dla każdej pary bitów $a$, $b$
        w~wektorach $A$~i~$B$ wartości zgodnie z~poniższą tabelką:
        ...
        %todo: opisz rotację i przesunięcia bitowe

        \begin{figure}
            \includegraphics[width=2cm]{logo-uam/logo-uam.png}
            %todo: faktyczna operacja md5 a nie placeholder
            \caption{Jedna operacja \texttt{MD5}.}
            \label{fig:md5_operation}
        \end{figure}

    \end{myenumerate}

    \item Po wykonaniu wszystkich 64~operacji (4~rundy $\times$
    16~operacji), stan wewnętrzny $(A,B,C,D)$ jest zwracany jako wyjściowy
    skrót z~funkcji.
