...
\item \refstepcounter{sccounter}\label{wl:ngrams_counter} Skrypt do analizy
częstościowej występowania liter, digramów oraz trigramów w tekście źródłowym.
Język: Python; użycie: \texttt{./ngrams.py ŹRÓDŁO} gdzie \texttt{ŹRÓDŁO}
stanowi ścieżkę do pliku zawierającego tekst źródłowy.
\begin{verbatim}
#!/usr/bin/python
import sys
import re
...
class Processor(object):
    def __init__(self, file):
        ...
        for n in self.nums:
            self.ngrams[n] = {}
            for key in itertools.product(self.alpha, repeat=n):
                key = ''.join(key)
                self.ngrams[n][key] = 0
    ...
    def run(self):
        ...
        total = dict((n, sum(self.ngrams[n].values())) for n in self.nums)
        pad1 = max(len(str(n)) for n in self.nums)
        pad2 = max(len(str(total[n])) for n in self.nums)
        format = '%' + str(pad1) + 's: %' + str(pad2) + 'd (%.05f%%)'
        for n, ngram in self.ngrams.iteritems():
            percent_multiplier = 100.0 / total[n]
            keys = sorted(ngram, key=ngram.get, reverse=True)
            for key in keys:
                value = ngram[key]
                if value > 0:
                    percent = value * percent_multiplier
