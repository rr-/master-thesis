...
\usepackage{caption} %multi-line captions
...
został przedstawiony poniżej (wszystkie operacje są dokonywane w~porządku
bitowym \en{little endian}; znaczenie operatorów wprowadzonych w~tej sekcji
zostało opisane w~dodatku~\ref{app:bitwise_operations}.
...
\begin{myenumerate}
    ...
    \begin{myenumerate}
        ...
        \ldots, M_n$ będziemy aktualizować $(A,B,C,D)$
        wartością funkcji kompresji obliczoną na
        podstawie danego bloku i~poprzedniego stanu:
        $(A,B,C,D) := \mathtt{Comp}((A,B,C,D),M_i)$.

        Sama funkcja kompresji składa się z~4~rund. W~każdej rundzie wykonywane
        jest 16~operacji przekształcających stan $(A,B,C,D)$ -- konstrukcja
        pojedynczej operacji została przedstawiona na
        rysunku~\ref{fig:md5_operation}. Każda operacja wykorzystuje funkcję
        $F$, która jest zależna od aktualnej rundy zgodnie
...
\begin{figure}
    ...
    \begin{itemize}
        ...
        \item $M_i$ -- 32-bitowy fragment 512-bitowego bloku wiadomości,
...
\begin{table}
    \caption{Funkcja $F$ użyta w~kolejnych rundach.}
    \label{tbl:md5_round_function}
    \begin{tabular}{|l|l|}
        \hline
        Runda & Funkcja rundy \\
        \hline
        Runda 1 & $F(B,C,D) = (B \land C) \lor (\lnot B \land D)$ \\
        Runda 2 & $F(B,C,D) = (B \land D) \lor (C \land \lnot D)$ \\
        Runda 3 & $F(B,C,D) = B \oplus C \oplus D$ \\
        Runda 4 & $F(B,C,D) = C \oplus (B \lor \lnot D)$ \\
        \hline
    \end{tabular}
\end{table}
...
\begin{table}[thb]
    ...
    podczas działania funkcji kompresji.}
